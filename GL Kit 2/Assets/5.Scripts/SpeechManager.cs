...
public class SpeechManager : BetterMonoBehaviour
{
	...
	private void OnFinishRoomTransition(FinishedRoomTransition info)
	{
		...
		{
			Debug.Log("Just focused Room " + RoomManager.Instance.GetCurrentRoomID().z);
			
			FillSpeechBubbleEvent newInfo = new FillSpeechBubbleEvent(dialogueObjects[RoomManager.Instance.GetCurrentRoomID().z], Settings.VAL_SPEECH_BUBBLE_TRANSITION_SECONDS, UIAnimator.MoveType.TRANSITION, UIAnimator.BlurType.IN, SpeechBubble.FillTextMethod.ITERATE, true);
