...
public class DragAndDrop : BetterMonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public void OnBeginDrag(PointerEventData eventData)
    {
        if (isObjectInGrid)
        {
            ...
            DropZone dropZone = DropZoneCheck(CachedTransform as RectTransform);
    ...
    private DropZone DropZoneCheck(RectTransform rectTransform)
    {
        foreach (DropZone dropZone in GridHandler.Instance.GetDropZonesUnder(rectTransform))
        {
            if(layer == dropZone.Layer)
            {
                return dropZone;
            }
        }
